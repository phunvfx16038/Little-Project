...
const ListCards = () => {
      ...
      const handleClickArrow = (direction:string)=>{
            let distance = listRef.current?.getBoundingClientRect().x - 160;
  ...
  <div>
      ...
      <span className='arrow-left' onClick={()=>handleClickArrow("left")}><BiLeftArrow/></span>
      <Row gutter={8} style={{width:"100%"}} className='cover-list' ref = {listRef}>
      {
            eventList.map((card)=>(
      ...
      <span className='arrow-right'onClick={()=>handleClickArrow("right")}><BiRightArrow/></span>
      </div>
  </div>
  )
}
